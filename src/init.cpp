 ...
 * between toschaind, and toschain-qt and non-server tools.
*/
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file. Relative paths will be prefixed by datadir location. (default: %s)"), TOSCHAIN_CONF_FILENAME));
    if (mode == HMM_TOSCHAIND)
    {
#if HAVE_DECL_DAEMON
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file. Relative paths will be prefixed by a net-specific datadir location. (default: %s)"), TOSCHAIN_PID_FILENAME));
#endif
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/toschain/toschain>";
    const std::string URL_WEBSITE = "<https://toschaincore.org>";
...
/** Sanity checks
 *  Ensure that Toschain is running in a usable environment with all
...
bool AppInitParameterInteraction()
{
    ...
    int64_t nMempoolSizeMin = gArgs.GetArg("-limitdescendantsize", DEFAULT_DESCENDANT_SIZE_LIMIT) * 1000 * 40; //limitdescendantsizeÈç¹ûÄ³¸ö½»Ò×ÔÚmempoolÖÐËùÓÐµÄ×æÏÈsizeÖ®ºÍ³¬¹ý¸ÃÏÞÖÆÖµ£¬ÄÇÃ´Ôò¾Ü¾ø½ÓÊÜ¸Ã½»Ò×¡£µ¥Î»ÎªKB¡£
...
{
    // Make sure only a single Toschain process is using the data directory.
...
bool AppInitSanityChecks()
{
    ...
    // Initialize elliptic curve code  //ÍÖÔ²ÇúÏß²ÎÊý³õÊ¼»¯
...
bool AppInitMain()
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", TOSCHAIN_CONF_FILENAME)).string());
    LogPrintf("Using at most %i automatic connections (%i file descriptors available)\n", nMaxConnections, nFD);

    // Warn about relative -datadir path.
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the "
                  "current working directory '%s'. This is fragile, because if toschain is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if toschain is started while in a temporary directory.\n",
