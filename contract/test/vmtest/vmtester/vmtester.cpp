...
extern "C" using tvmc_create_fn = tvmc_instance * ();

namespace
{
boost::function<tvmc_create_fn> create_fn;

std::unique_ptr<tvmc_instance, tvmc_destroy_fn> create_vm()
{
    auto vm = create_fn();
...
int main(int argc, char* argv[])
{
    try
    {
        ...
        opts::options_description desc("tvmc VM Tester Options");
        ...
        auto it = std::find_if(symbols.begin(), symbols.end(),
            [](const std::string& symbol) { return symbol.find("tvmc_create_") == 0; });
        if (it == symbols.end())
        {
            std::cerr << "tvmc create function not found in " << vm_path.string() << "\n";
            return 2;
        }
        ...
        create_fn = dll::import<tvmc_create_fn>(vm_path, create_fn_name);
