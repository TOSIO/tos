...
using tvmc_create_fn = tvmc_instance*();
...
boost::function<tvmc_create_fn> loadDllVM(fs::path _path)
{
    if (!fs::exists(_path))
    {
    ...
    static const auto predicate = [](const std::string& symbol) {
        return symbol.find("tvmc_create_") == 0;
    };
    ...
    if (it == symbols.end())
    {
        ...
        std::string what = "loading " + _path.string() + " failed: tvmc create function not found";
        BOOST_THROW_EXCEPTION(std::system_error(ec, what));
    }

    // Check for ambiguity of tvmc create functions.
    auto dupIt = std::find_if(std::next(it), symbols.end(), predicate);
    if (dupIt != symbols.end())
    {
        auto ec = std::make_error_code(std::errc::invalid_seek);
        std::string what = "loading " + _path.string() +
                           " failed: multiple tvmc create functions: " + *it + " and " + *dupIt;
        BOOST_THROW_EXCEPTION(std::system_error(ec, what));
    }
    return dll::import<tvmc_create_fn>(_path, *it);
}
...
void setVMKind(const std::string& _name)
{
    ...
    g_dllTvmcCreate = loadDllVM(_name);
    g_kind = VMKind::DLL;
}
}  // namespace
...
#ifdef ETH_EVMJIT
    case VMKind::JIT:
        return std::unique_ptr<VMFace>(new TVMC{tvmjit_create()});
#endif
#ifdef ETH_HERA
    case VMKind::Hera:
        return std::unique_ptr<VMFace>(new TVMC{tvmc_create_hera()});
#endif
    case VMKind::Interpreter:
        return std::unique_ptr<VMFace>(new TVMC{tvmc_create_interpreter()});
    case VMKind::DLL:
        return std::unique_ptr<VMFace>(new TVMC{g_dllTvmcCreate()});
    case VMKind::Legacy:
    default:
