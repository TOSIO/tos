...
extern "C" tvmc_instance* tvmc_create_interpreter() noexcept
{
...
namespace
{
void destroy(tvmc_instance* _instance)
{
    delete static_cast<dev::eth::VM*>(_instance);
}

void delete_output(const tvmc_result* result)
{
    delete[] result->output_data;
}

tvmc_result execute(tvmc_instance* _instance, tvmc_context* _context, tvmc_revision _rev,
    const tvmc_message* _msg, uint8_t const* _code, size_t _codeSize) noexcept
{
    ...
    try
    {
        ...
        result.status_code = TVMC_SUCCESS;
        result.gas_left = vm->m_io_gas;
    }
    catch (dev::eth::RevertInstruction& ex)
    {
        result.status_code = TVMC_REVERT;
        result.gas_left = vm->m_io_gas;
    ...
    {
        result.status_code = TVMC_INTERNAL_ERROR;
    }
...
VM::VM()
  : tvmc_instance{
        TVMC_ABI_VERSION,
        "interpreter",
        tos_get_buildinfo()->project_version,
        ::destroy,
        ::execute,
        nullptr,
    }
...
void VM::updateSSGas()
{
    tvmc_uint256be key = toTvmC(m_SP[0]);
    tvmc_uint256be rawValue;
    m_context->fn_table->get_storage(&rawValue, m_context, &m_message->destination, &key);
    u256 value = fromTvmC(rawValue);
...
tvmc_tx_context const& VM::getTxContext()
{
    if (!m_tx_context)
    {
...
owning_bytes_ref VM::exec(tvmc_context* _context, tvmc_revision _rev, const tvmc_message* _msg,
...
void VM::interpretCases()
{
    ...
    DO_CASES
    {
        ...
        {
            ON_OP();
            if (m_message->flags & TVMC_STATIC)
        ...
        {
            ON_OP();
            if (m_OP == Instruction::DELEGATECALL && m_rev < TVMC_HOMESTEAD)
                throwBadInstruction();
            if (m_OP == Instruction::STATICCALL && m_rev < TVMC_BYZANTIUM)
                throwBadInstruction();
            if (m_OP == Instruction::CALL && m_message->flags & TVMC_STATIC && m_SP[2] != 0)
                throwDisallowedStateChange();
        ...
        CASE(SUICIDE)
        {
            ...
            m_runGas = m_rev >= TVMC_TANGERINE_WHISTLE ? 5000 : 0;
            tvmc_address destination = toTvmC(asAddress(m_SP[0]));

            // After EIP158 zero-value suicides do not have to pay account creation gas.
            tvmc_uint256be rawBalance;
            m_context->fn_table->get_balance(&rawBalance, m_context, &m_message->destination);
            u256 balance = fromTvmC(rawBalance);
            if (balance > 0 || m_rev < TVMC_SPURIOUS_DRAGON)
            {
                ...
                if (m_rev >= TVMC_TANGERINE_WHISTLE && !destinationExists)
        ...
        CASE(LOG1)
        {
            ...
            tvmc_uint256be topics[] = {toTvmC(m_SP[2])};
        ...
        CASE(LOG2)
        {
            ...
            tvmc_uint256be topics[] = {toTvmC(m_SP[2]), toTvmC(m_SP[3])};
            size_t numTopics = sizeof(topics) / sizeof(topics[0]);
        ...
        CASE(LOG3)
        {
            ...
            tvmc_uint256be topics[] = {toTvmC(m_SP[2]), toTvmC(m_SP[3]), toTvmC(m_SP[4])};
            size_t numTopics = sizeof(topics) / sizeof(topics[0]);
        ...
        CASE(LOG4)
        {
            ...
            tvmc_uint256be topics[] = {
                toTvmC(m_SP[2]), toTvmC(m_SP[3]), toTvmC(m_SP[4]), toTvmC(m_SP[5])};
            size_t numTopics = sizeof(topics) / sizeof(topics[0]);
        ...
        {
            u256 expon = m_SP[1];
            const int64_t byteCost = m_rev >= TVMC_SPURIOUS_DRAGON ? 50 : 10;
        ...
        CASE(ADDRESS)
        {
            ...
            m_SPP[0] = fromAddress(fromTvmC(m_message->destination));
        }
        NEXT
        ...
        CASE(ORIGIN)
        {
            ...
            m_SPP[0] = fromAddress(fromTvmC(getTxContext().tx_origin));
        }
        NEXT

        CASE(BALANCE)
        {
            m_runGas = m_rev >= TVMC_TANGERINE_WHISTLE ? 400 : 20;
            ON_OP();
            updateIOGas();

            tvmc_address address = toTvmC(asAddress(m_SP[0]));
            tvmc_uint256be rawBalance;
            m_context->fn_table->get_balance(&rawBalance, m_context, &address);
            m_SPP[0] = fromTvmC(rawBalance);
        }
        NEXT
        ...
        CASE(CALLER)
        {
            ...
            m_SPP[0] = fromAddress(fromTvmC(m_message->sender));
        }
        NEXT
        ...
        CASE(CALLVALUE)
        {
            ...
            m_SPP[0] = fromTvmC(m_message->value);
        }
        NEXT
        ...
        CASE(EXTCODESIZE)
        {
            m_runGas = m_rev >= TVMC_TANGERINE_WHISTLE ? 700 : 20;
            ON_OP();
            updateIOGas();
        ...
        CASE(GASPRICE)
        {
            ...
            m_SPP[0] = fromTvmC(getTxContext().tx_gas_price);
        }
        NEXT
        ...
        CASE(COINBASE)
        {
            ...
            m_SPP[0] = fromAddress(fromTvmC(getTxContext().block_coinbase));
        }
        NEXT
        ...
        CASE(DIFFICULTY)
        {
            ...
            m_SPP[0] = fromTvmC(getTxContext().block_difficulty);
        }
        NEXT
        ...
        CASE(SLOAD)
        {
            m_runGas = m_rev >= TVMC_TANGERINE_WHISTLE ? 200 : 50;
            ON_OP();
            updateIOGas();

            tvmc_uint256be key = toTvmC(m_SP[0]);
            tvmc_uint256be value;
            m_context->fn_table->get_storage(&value, m_context, &m_message->destination, &key);
            m_SPP[0] = fromTvmC(value);
        }
        NEXT
        ...
        CASE(SSTORE)
        {
            ...
            tvmc_uint256be key = toTvmC(m_SP[0]);
            tvmc_uint256be value = toTvmC(m_SP[1]);
