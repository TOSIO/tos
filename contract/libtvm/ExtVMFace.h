...
///
/// @todo: Replace with tvmc_result in future.
struct CallResult
{
    tvmc_status_code status;
    owning_bytes_ref output;

    CallResult(tvmc_status_code status, owning_bytes_ref&& output)
...
struct CreateResult
{
    ...
    CreateResult(tvmc_status_code status, owning_bytes_ref&& output, h160 const& address)
 ...
 */
class ExtVMFace: public tvmc_context
{
public:
    /// Full constructor.
...
inline tvmc_address toTvmC(Address const& _addr)
{
    return reinterpret_cast<tvmc_address const&>(_addr);
}

inline tvmc_uint256be toTvmC(h256 const& _h)
{
    return reinterpret_cast<tvmc_uint256be const&>(_h);
}

inline u256 fromTvmC(tvmc_uint256be const& _n)
{
    return fromBigEndian<u256>(_n.bytes);
}

inline Address fromTvmC(tvmc_address const& _addr)
{
