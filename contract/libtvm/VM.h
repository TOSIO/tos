...
class VM : public tvmc_instance
{
public:
    VM();

    owning_bytes_ref exec(tvmc_context* _context, tvmc_revision _rev, const tvmc_message* _msg,
    ...
    uint64_t m_io_gas = 0;
private:
    tvmc_context* m_context = nullptr;
    tvmc_revision m_rev = TVMC_FRONTIER;
    tvmc_message const* m_message = nullptr;
    boost::optional<tvmc_tx_context> m_tx_context;
