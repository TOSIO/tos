...
void VM::caseCreate()
{
    ...
    if (balance >= endowment && m_message->depth < 1024)
    {
        tvmc_message msg = {};
        msg.gas = m_io_gas;
        if (m_rev >= TVMC_TANGERINE_WHISTLE)
            msg.gas -= msg.gas / 64;
        ...
        msg.kind = TVMC_CREATE;  // FIXME: In EVM-C move the kind to the top.
        msg.value = toTvmC(endowment);

        tvmc_result result;
        m_context->fn_table->call(&result, m_context, &msg);

        if (result.status_code == TVMC_SUCCESS)
            m_SPP[0] = fromAddress(fromTvmC(result.create_address));
        else
            m_SPP[0] = 0;
...
void VM::caseCall()
{
    ...
    if (caseCallSetup(msg, output))
    {
        ...
        m_SPP[0] = result.status_code == TVMC_SUCCESS ? 1 : 0;
...
bool VM::caseCallSetup(tvmc_message& o_msg, bytesRef& o_output)
{
    m_runGas = m_rev >= TVMC_TANGERINE_WHISTLE ? 700 : 40;
    ...
    default:
        o_msg.kind = TVMC_CALL;
        break;
    case Instruction::CALLCODE:
        o_msg.kind = TVMC_CALLCODE;
        break;
    case Instruction::DELEGATECALL:
        o_msg.kind = TVMC_DELEGATECALL;
        break;
    }

    if (m_OP == Instruction::STATICCALL || m_message->flags & TVMC_STATIC)
        o_msg.flags = TVMC_STATIC;

    bool const haveValueArg = m_OP == Instruction::CALL || m_OP == Instruction::CALLCODE;

    tvmc_address destination = toTvmC(asAddress(m_SP[1]));
    int destinationExists = m_context->fn_table->account_exists(m_context, &destination);

    if (m_OP == Instruction::CALL && !destinationExists)
    {
        if (m_SP[2] > 0 || m_rev < TVMC_SPURIOUS_DRAGON)
