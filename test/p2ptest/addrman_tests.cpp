...
public:
    ...
    bool compare(const CAddrManTest& other)
    {
        if (nKey.ToString() != other.nKey.ToString()){
            printf("Trace | bool compare(const CAddrManTest& other) nKey\n");
            return false;
        }
        if (nNew != other.nNew)
        {
            printf("Trace | bool compare(const CAddrManTest& other) nNew\n");
            return false;
        }
        if (nTried != other.nTried)
        {
            printf("Trace | bool compare(const CAddrManTest& other) nTried\n");
            return false;
        }
        if (mapInfo.size() != other.mapInfo.size())
        {
            printf("Trace | bool compare(const CAddrManTest& other) mapInfo\n");
            return false;
        }
        for (auto kv : mapInfo)
        {
            if (other.mapInfo.find(kv.first) == other.mapInfo.end())
            {
                printf("Trace | bool compare(const CAddrManTest& other) mapInfo element\n");
                return false;
            }
        }
        ...
        {
            for (int j= 0; j < ADDRMAN_BUCKET_SIZE; ++i)
            {
                if (vvNew[i][j] != other.vvNew[i][j])
                {
                    printf("Trace | bool compare(const CAddrManTest& other) vvNew : [%d,%d]=%d,other : %d \n",i,j,vvNew[i][j],other.vvNew[i][j]);
                    return false;
                }
            }
        }
        for (int i = 0; i < ADDRMAN_TRIED_BUCKET_COUNT; ++i)
        {
            for (int j= 0; j < ADDRMAN_BUCKET_SIZE; ++i)
            {
                if (vvTried[i][j] != other.vvTried[i][j])
                {
                    printf("Trace | bool compare(const CAddrManTest& other) vvTried : [%d,%d]\n",i,j);
                    return false;
                }
            }
        }
...
BOOST_AUTO_TEST_CASE(addrman_serialize)
{
    ...
    printf("Trace | BOOST_AUTO_TEST_CASE(addrman_serialize) processing serialize...\n");
    DataStream stream(SER_DISK,CLIENT_VERSION);
    addrman.Serialize(stream);
    printf("Trace | BOOST_AUTO_TEST_CASE(addrman_serialize) make output...\n");
    bytes encStream = stream.stream()->out();
    printf("Trace | BOOST_AUTO_TEST_CASE(addrman_serialize) processing unserialize...\n");

    CAddrManTest decAddrMan;
    decAddrMan.UnSerialize(&encStream,SER_DISK,CLIENT_VERSION);
    printf("Trace | BOOST_AUTO_TEST_CASE(addrman_serialize) processing unserialize completed.\n");
    BOOST_CHECK_EQUAL(addrman.size(), decAddrMan.size());
    BOOST_CHECK(addrman.compare(decAddrMan) == true);
...
BOOST_AUTO_TEST_CASE(addrman_new_collisions)
{
    ...
    printf("Trace | BOOST_AUTO_TEST_CASE(addrman_new_collisions)\n");
